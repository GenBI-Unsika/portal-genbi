...
export default function ToastProvider({ children }) {
  ...
  return (
    <ToastCtx.Provider value={{ push, remove }}>
      {children}
      <div className="fixed top-4 right-4 z-[9999] flex w-[320px] flex-col gap-2">
        {items.map((t) => (
          <div key={t.id} className={`card shadow-soft-sm border ${palette[t.type]}`}>
            <div className="p-3 text-sm flex items-start justify-between gap-3">
              <div className="pr-1 flex-1">{t.message}</div>
              <button className="p-1 opacity-60 hover:opacity-100 transition-opacity rounded-md hover:bg-black/5" onClick={() => remove(t.id)} aria-label="Tutup">
                <X className="w-4 h-4" />
              </button>
            </div>
          </div>
