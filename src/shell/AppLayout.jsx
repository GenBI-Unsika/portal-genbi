...
export default function AppLayout() {
  ...
  const [collapsed, setCollapsed] = useState(() =>
    JSON.parse(localStorage.getItem("sb-col") || "false")
  );
    ...
    <ToastProvider>
      <div className="min-h-screen bg-neutral-50 text-neutral-800">
        <div className="flex">
          {/* Sidebar */}
          <aside
            className={`sticky top-0 flex h-screen flex-col border-r border-neutral-200 bg-white transition-[width] duration-200 ${
          ...
          <div className="flex min-h-screen flex-1 flex-col">
            <header className="sticky top-0 z-10 border-b border-neutral-200 bg-white/90 backdrop-blur shadow-soft-sm">
              <div className="flex items-center gap-3 px-4 py-3 md:px-6">
                <button
                  className="inline-grid h-9 w-9 place-items-center rounded-lg border border-neutral-200 hover:bg-neutral-50 md:hidden focus-ring"
                  onClick={() => setCollapsed((s) => !s)}
                  aria-label="Toggle sidebar"
                >
